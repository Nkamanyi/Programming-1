...
def add_friendship(network, name1, name2):
    ...
    #       already known to be friends, nothing needs to be done.
        ...
        if name2 not in network[name1]:
            network.setdefault(name1, []).append(name2)
            network.setdefault(name2, []).append(name1)
...
def print_command(network, namelist):
    ...
    :param network: dict, Contains the friendship network to be printed.
    ...
    for na, nam in sorted(network.items()):
        print(na)
        for friends in sorted(nam):
            print(f"- {friends}")
...
def common_friends_command(network, namelist):
    ...
    for na, nam in network.items():
        if na == name1:
            for friends in sorted(nam):
                friends_of_name1.append(friends)

    for na, nam in network.items():
        if na == name2:
            for friends in sorted(nam):
                friends_of_name2.append(friends)

    set_of_common_friends = set(friends_of_name1)
    common_friends = set_of_common_friends.intersection(friends_of_name2)

    if len(common_friends) < 1:
        print(f"{name1} and {name2} have no common friends.")
        return

    for namez in sorted(common_friends):
        print(namez)
