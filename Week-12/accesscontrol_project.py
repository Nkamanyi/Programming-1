...
class Accesscard:
    ...
    def add_access(self, new_access_code):
        ...
        # TODO: Implement the method
        if new_access_code in self.__access_codes:  # checks if it already exists
            return
            ...
            for code in self.__access_codes:
                if code in DOORCODES:  # is a door
                    for area in DOORCODES[code]:  # iterate through the doors areas..
                        if area in self.__access_codes:  # checks if area already included
    ...
    def check_access(self, door):
        ...
        # TODO: Implement the method
        if door in self.__access_codes:     # if door is already there
            return True
        for c in DOORCODES[door]:   # if any of the areacodes in the door are already included
    ...
    def merge(self, card):
        ...
        # TODO: Implement the method
        new_codes = card.get_access_codes()     # retrieve the codes from the merging card
        for code in new_codes:
            if code not in self.__access_codes:     # check if already in the the card
                self.add_access(code)

    # additional methods
    def get_access_codes(self):
        """
        Returns the access codes of the card holder

        :return: list, the accesscodes of the card holder
        """
        return self.__access_codes

    def access_codes_to_string(self):
        """
        Returns the access codes of a card holder as a string

        :return: str, access codes of the card holder as a string
        """
...
def read_file(file_name):
    ...
    :return: dict, dictionary containing Accesscard objects
    """
